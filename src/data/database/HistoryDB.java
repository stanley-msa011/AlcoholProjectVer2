...
public class HistoryDB {
	...
	public DateBracDetectionState[] getAllHistory() {
		...
		for (int i = 0; i < historys.length; ++i) {
			...
			historys[i] = new DateBracDetectionState(week, ts, year, month, day, tb, brac, emotion,
					desire);
		}
		cursor.close();
		db.close();
	...
	public DateBracDetectionState getLatestBracDetection() {
		...
		Cursor cursor = db
				.rawQuery(
						"SELECT brac, ts, year, month, day, timeblock, week, emotion, desire FROM Detection ORDER BY id DESC LIMIT 1",
						null);
	...
	public AccumulatedHistoryState getLatestAccumulatedHistoryState() {
		...
		Cursor cursor = db
				.rawQuery(
						"SELECT "
								...
								+ " FROM AccDetection AS a, Detection AS d WHERE a.detection_id = d.id ORDER BY a.id DESC LIMIT 1",
						null);
		...
		cursor.close();
		db.close();
		AccumulatedHistoryState a = new AccumulatedHistoryState(week, w_accTest, w_accPass,
				accTest, accPass);
		return a;
	}
	...
	public AccumulatedHistoryState[] getAccumulatedHistoryStateByWeek() {
		...
		String sql = "SELECT " + " a.w_morning,a.w_noon,a.w_night,"
				+ " a.w_morning_pass,a.w_noon_pass,a.w_night_pass," + " d.week "
				+ " FROM AccDetection AS a, Detection AS d WHERE a.detection_id = d.id AND week <="
				+ curWeek + " GROUP BY d.week";
	...
	public void insertNewState(DateBracDetectionState state, AccumulatedHistoryState a_state) {
		...
		sql = "INSERT INTO AccDetection (" + " detection_id," + " w_morning,w_noon,w_night,"
				+ " morning,noon,night," + " w_morning_pass,w_noon_pass,w_night_pass,"
				+ " morning_pass,noon_pass,night_pass" + ") VALUES ("
				+ did
				+ ","
				+ accTest[0]
				+ ","
	...
	public BracDetectionState[] getTodayBracState() {
		...
		for (int i = 0; i < nBlocks; ++i) {
			if (!TimeBlock.hasBlock(i))
				continue;
			String sql = "SELECT brac FROM Detection WHERE " + " year=" + year + " AND month="
					+ month + " AND day=" + day + " AND timeblock=" + i
	...
	public BracDetectionState[] getMultiDayInfo(int n_days) {
		...
		String sql = "SELECT * FROM Detection JOIN ("
				+ " SELECT MIN(id) AS mid FROM Detection WHERE ts >= " + start_ts
				+ " GROUP BY year,month,day,timeblock ORDER BY id ASC" + ")";
	...
	public BracDetectionState[] getAllNotUploadedDetection() {
		...
		if (cur_ts <= gps_ts) {
			...
			sql = "SELECT id,week, ts, brac, emotion, desire FROM Detection WHERE upload = 0 AND ts <> "
					+ gps_detection_ts + " ORDER BY id ASC";
		} else {
	...
	public RankHistoryDetail[] getAllUsersHistory() {
		RankHistoryDetail[] historys = null;
		db = dbHelper.getReadableDatabase();
		...
		historys = new RankHistoryDetail[count];
		int uid_idx = cursor.getColumnIndex("user_id");
		int level_idx = cursor.getColumnIndex("score");
		int t_idx = cursor.getColumnIndex("test_score");
		int q_idx = cursor.getColumnIndex("ques_score");
		int s_idx = cursor.getColumnIndex("story_score");
		for (int i = 0; i < count; ++i) {
			cursor.moveToPosition(i);
			String uid = cursor.getString(uid_idx);
			int score = cursor.getInt(level_idx);
			int test = cursor.getInt(t_idx);
			int ques = cursor.getInt(q_idx);
			int story = cursor.getInt(s_idx);
			historys[i] = new RankHistoryDetail(score, uid,test,ques,story);
		}
		cursor.close();
		db.close();
		return historys;
	...
	public void insertInteractionHistory(RankHistoryDetail history) {
		db = dbHelper.getWritableDatabase();
		String sql = "SELECT * FROM Ranking WHERE user_id = '" + history.uid + "'";
		Cursor cursor = db.rawQuery(sql, null);
		if (cursor.getCount() == 0) {
			sql = "INSERT INTO Ranking (user_id,score,test_score,ques_score,story_score) VALUES ('"
					+ history.uid + "'," + history.score + "," + history.test + "," + history.ques
					+ "," + history.story + ")";
			db.execSQL(sql);
		} else {
			sql = "UPDATE Ranking SET score = " + history.score + ", test_score = " + history.test
					+ " , ques_score = " + history.ques + " , story_score = " + history.story
					+ " WHERE user_id ='" + history.uid + "'";
			db.execSQL(sql);
		}
		cursor.close();
		db.close();
	...
	public void insertInteractionHistoryToday(RankHistory history) {
		...
		Cursor cursor = db.rawQuery(sql, null);
		if (cursor.getCount() == 0) {
			sql = "INSERT INTO RankingToday (user_id,score) VALUES ('" + history.uid + "',"
					+ history.score + ")";
			db.execSQL(sql);
		} else {
			sql = "UPDATE RankingToday SET score = " + history.score + " WHERE user_id ='"
					+ history.uid + "'";
			db.execSQL(sql);
		}
		cursor.close();
	...
	public boolean getIsDone(Calendar curCal) {
		...
		String sql = "SELECT id FROM Detection WHERE year =" + year + " AND month = " + month
				+ " AND day= " + day + " AND timeblock= " + time_block;
		Cursor cursor = db.rawQuery(sql, null);
	...
	public void restoreUsedDetection(UsedDetection usedDetection) {// FOR
																	// RESTORE
		db = dbHelper.getWritableDatabase();
