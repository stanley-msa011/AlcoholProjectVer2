...
public class PreSettingActivity extends Activity {
	...
	private CheckBox uploadAudioCheckBox, showSavingBox, autoCleanBox,uploadFBCheckBox;

	private EditText recreation0, recreation1, recreation2, recreation3,
			recreation4;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		target_good.setText(String.valueOf(sp.getString("goal_good",
				getString(R.string.default_goal_good))));
		...
		adapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item, ConnectSocialInfo.NAME);
		...
		lYear = sp.getInt("lockYear", c.get(Calendar.YEAR));
		lMonth = sp.getInt("lockMonth", c.get(Calendar.MONTH));
		lDay = sp.getInt("lockDate", c.get(Calendar.DATE));
		...
		uploadFBCheckBox = (CheckBox)findViewById(R.id.upload_facebook_check_box);
		uploadFBCheckBox.setChecked(sp.getBoolean("FacebookText", false));
		...
		recreation0.setText(sp.getString("recreation0",
				getString(R.string.default_recreation_1)));
		recreation1.setText(sp.getString("recreation1",
				getString(R.string.default_recreation_2)));
		recreation2.setText(sp.getString("recreation2",
				getString(R.string.default_recreation_3)));
		recreation3.setText(sp.getString("recreation3", ""));
		recreation4.setText(sp.getString("recreation4", ""));

		autoCleanBox = (CheckBox) findViewById(R.id.auto_delete);
		autoCleanBox.setChecked(sp.getBoolean("auto_clean", true));
		...
		lDateCheckBox.setChecked(sp.getBoolean("system_lock", false));
		...
		clean_button.setOnClickListener(new AlertOnClickListener(
				cleanAlertDialog));
	...
	private class OKOnclickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (check) {
				...
				editor.putBoolean("upload_audio",
						uploadAudioCheckBox.isChecked());
				editor.putBoolean("FacebookText", uploadFBCheckBox.isChecked());
				editor.putBoolean("show_saving", showSavingBox.isChecked());
				editor.putBoolean("auto_clean", autoCleanBox.isChecked());
				editor.putBoolean("system_lock", lDateCheckBox.isChecked());
				if (lDateCheckBox.isChecked()){
					editor.putInt("lockYear", lYear);
					editor.putInt("lockMonth", lMonth);
					editor.putInt("lockDate", lDay);
				}
				editor.commit();
