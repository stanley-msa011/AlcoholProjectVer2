...
public class EmotionManageActivity extends Activity {
	...
	private static final int[] EMOTION_DRAWABLE_ID = { R.drawable.questionnaire_item_e1,
	...
	private static final int[] RELATED_DRAWABLE_ID = { R.drawable.questionnaire_item_c1,
			R.drawable.questionnaire_item_c2, R.drawable.questionnaire_item_c3,
	...
	private void setQuestionEmotion() {
		...
		for (int i = 0; i < emotion_texts.length; ++i) {
			View v = createIconView(emotion_texts[i], EMOTION_DRAWABLE_ID[i],
					new EmotionOnClickListener(i + 1));
	...
	private void setQuestionType() {
		...
		for (int i = 0; i < related_texts.length; ++i) {
			View vv = createIconView(related_texts[i], RELATED_DRAWABLE_ID[i],
					new RelatedOnClickListener(i + 1));
	...
	private void setQuestionEdit() {
		...
		String str = getResources().getString(R.string.emotion_manage_help3)
				+ related_texts[r_type - 1];
		...
		View vv = createIconView(R.string.ok, R.drawable.questionnaire_item_ok,
				new EditedOnClickListener());
		mainLayout.addView(vv);
	...
	private void setQuestionEnd() {
		...
		mainLayout.addView(tv);
		View vv = createIconView(R.string.done, R.drawable.questionnaire_item_ok,
	...
	private View createEditView(int type) {
		...
		if (Build.VERSION.SDK_INT >= 16)
			edit.setBackground(mainLayout.getContext().getResources()
					.getDrawable(R.drawable.questionnaire_input));
		else
			edit.setBackgroundDrawable(mainLayout.getContext().getResources()
					.getDrawable(R.drawable.questionnaire_input));
		...
		if (select_item != null) {
			...
			RelativeLayout.LayoutParams uparam = (RelativeLayout.LayoutParams) updown
					.getLayoutParams();
	...
	private class EditedOnClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			if (r_texts.getText().toString().length() == 0) {
				CustomToastSmall.generateToast(getBaseContext(),
						R.string.emotion_manage_check_toast);
				return;
			}
			ClickLogger.Log(getBaseContext(), ClickLogId.EMOTIONMANAGE_SELECTION);
			state = 3;
		...
		public void onClick(View v) {
			...
			if (isShow) {
				...
				for (int i = 0; i < select_item.length; ++i) {
					View vv = createIconView(select_item[i], 0, new ChangeTextOnClickListener(
							select_item[i]));
